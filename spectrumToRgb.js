...
function xyz_to_rgb(colourSystem, x, y, z) {
    let rw, gw, bw;

    let xr = colourSystem.xRed;
    let yr = colourSystem.yRed;
    let zr = 1 - (xr + yr);

    let xg = colourSystem.xGreen;
    let yg = colourSystem.yGreen;
    let zg = 1 - (xg + yg);

    let xb = colourSystem.xBlue;
    let yb = colourSystem.yBlue;
    let zb = 1 - (xb + yb);

    let xw = colourSystem.whitePoint.x;
    let yw = colourSystem.whitePoint.y;
    let zw = 1 - (xw + yw);
...
function norm_rgb({r, g, b}) {
    const greatest = Math.max(r, Math.max(g, b));
...
function spectrum_to_xyz(spectrumIntensities) {
    let i;
    let X = 0, Y = 0, Z = 0;

    for (i = 0; i < cie_colour_match.length; i++) {
        X += spectrumIntensities[i] * cie_colour_match[i][0];
        Y += spectrumIntensities[i] * cie_colour_match[i][1];
        Z += spectrumIntensities[i] * cie_colour_match[i][2];
    }
...
const getRgbFromSpectrum = (spectrum) => {
    const colourSystem = EBUsystem;

    let rgb;

    const {x, y, z} = spectrum_to_xyz(spectrum);
    rgb = xyz_to_rgb(colourSystem, x, y, z);
...
export default getRgbFromSpectrum;